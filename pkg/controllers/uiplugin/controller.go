...
func (rm resourceManager) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	conf := rm.buildOverlayConfig(ctx, plugin, logger)
	...
	if overlay != nil && pluginInfo != nil {
		...
		if plugin.Spec.Type == uiv1alpha1.TypeMonitoring {
			if err := rm.cleanupMonitoringResources(ctx, plugin, conf, currentObjects, logger); err != nil {
				logger.V(8).Info("monitoring cleanup error", "err", err)
			}
		}
...
func (rm resourceManager) registerPluginWithConsole(ctx context.Context, consoleName string) error {
	...
	clusterPlugins := append(cluster.Spec.Plugins, consoleName)

	cluster.Spec.Plugins = clusterPlugins

	if err := reconciler.NewMerger(cluster, consoleName).Reconcile(ctx, rm.k8sClient, rm.scheme); err != nil {
		return err
	}
...
func (rm resourceManager) buildOverlayConfig(ctx context.Context, plugin *uiv1alpha1.UIPlugin, logger logr.Logger) UIPluginBuildConfig {
...
func (rm resourceManager) cleanupMonitoringResources(ctx context.Context, plugin *uiv1alpha1.UIPlugin, conf UIPluginBuildConfig, currentObjects []client.Object, logger logr.Logger) error {
	...
	fullOverlay, _, _ := BuildUIPluginOverlay(fullPlugin, conf, logger)
	if fullOverlay == nil {
		return nil
	}
	...
	for _, obj := range currentObjects {
		currentSet[gvkNameIdentifier(obj)] = true
	}

	for _, obj := range fullObjects {
		if !currentSet[gvkNameIdentifier(obj)] {
			r := reconciler.NewDeleter(obj)
			if err := r.Reconcile(ctx, rm.k8sClient, rm.scheme); err != nil {
				logger.V(8).Info("cleanup delete failed", "object", gvkNameIdentifier(obj), "err", err)
			}
		}
	}
